...
#define B_STANCE_CHANGE_FAIL            GEN_6      // In Gen7+, Stance Change fails if the Pokémon is unable to use a move due to confusion, paralysis, etc.
...
#define B_MOODY_ACC_EVASION             GEN_7      // In Gen8+, Moody can no longer raise accuracy and evasion.
...
#define B_SYMBIOSIS_GEMS                GEN_6      // In Gen7+, Symbiosis passes an item after a Gem-boosted attack. Previously, items were passed before the Gem-boosted attack hit, making the item's effect apply.
...
// Other battle flags
#define B_FLAG_INVERSE_BATTLE       FLAG_EXPANSION_INVERSE_BATTLE     // If this flag is set, the battle's type chart inverted. For example, Fire is super effective against Water.
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_EXPANSION_AI_VS_AI_BATTLE     // If this flag is set, the player's Pokémon will be controlled by the AI in the next battle.
...
#define B_FLAG_TERA_ORB_NO_COST     FLAG_EXPANSION_TERA_ORB_NO_COST     // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In SV, this occurs after an event with Terapagos.
#define B_FLAG_SLEEP_CLAUSE         FLAG_EXPANSION_SLEEP_CLAUSE     // If this flag is set, sleep clause is enabled; if a Pokémon on the one side of the field is asleep, another one on that side cannot be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.
...
#define B_VAR_NO_BAG_USE         VAR_EXPANSION_NO_BAG_USE     // If set to 1, the ability to use the Bag is disabled in Trainer battles. If set to 2, it is also disabled in wild battles.
...
#define B_WAIT_TIME_MULTIPLIER              10         // This determines how long text pauses for in battle. Vanilla is 16. Lower values result in faster battles.
